<script lang="ts">
  ...
  // Remembered across restarts: a bridge only has to prove itself once.
  let browserEverConnected = localStorage.getItem("hotkey-browser-verified") === "1";
  let discordEverConnected = localStorage.getItem("hotkey-discord-verified") === "1";

  function noteBridgeVerified(kind: "browser" | "discord") {
    if (kind === "browser" && !browserEverConnected) {
      browserEverConnected = true;
      localStorage.setItem("hotkey-browser-verified", "1");
    }
    if (kind === "discord" && !discordEverConnected) {
      discordEverConnected = true;
      localStorage.setItem("hotkey-discord-verified", "1");
    }
  }
  ...
  // "Set up" and "connected right now" are different things. A bridge that has
  // worked before is simply idle when the browser or Discord is closed, and
  // nagging about that forever is noise. Once either has connected even once,
  // the prompt is done: it is there to get you through Setup, not to report
  ...
  function bindingRequirement(binding: BindingSpec) {
    ...
    if (DISCORD_ACTIONS.has(name) && !discordEverConnected) {
      return { kind: "discord", hard: true, text: "Needs the Discord bridge from Setup, or this does nothing." };
    }
    if (DISCORD_OPTIONAL_ACTIONS.has(name) && !discordEverConnected) {
    ...
    if (BROWSER_ACTIONS.has(name) && !browserEverConnected) {
  ...
  // Published listing. The item id is stable for the life of the extension.
  const WEB_STORE_URL =
    "https://chromewebstore.google.com/detail/jejgnmfgjblnacclebdhmonhljfcbfdo";
  ...
  function onboardingPrimaryText() {
    if (onboardingCurrent === "finish") return "Start using KeyRail";
...
<main data-theme={theme}>
  <header class="titlebar" role="presentation" data-tauri-drag-region on:pointerdown={startWindowDrag}>
    <div class="window-title" data-tauri-drag-region>
      ...
      <p data-tauri-drag-region>KeyRail</p>
    </div>
  ...
  <section class="shell">
    <aside class="sidebar">
      ...
      <section class="config-panel">
        ...
        <p>{configPath || "%APPDATA%\\KeyRail\\config.json"}</p>
      ...
      {#if activePage === "setup"}
        ...
        <section class="wiz-panel">
          ...
          {#if wizardStep === 1}
            ...
            {:else}
              <div class="wiz-task" class:done={extensionLive}>
                ...
                <div class="wiz-task-body">
                  <div class="wiz-task-title">
                    ...
                    {#if extensionLive}<span class="wiz-state ok">Installed</span>{/if}
                  </div>
                  <p>
                    One click from the Chrome Web Store. It works in Chrome, Brave, Edge and any other Chromium
                    browser.
                  </p>
                  <div class="wiz-actions">
                    <button class="primary-soft" on:click={() => openExternal(WEB_STORE_URL)}>
                      Open the store page
                    </button>
                  </div>
                </div>
              ...
              <div class="wiz-task" class:muted={extensionLive} class:done={extensionLive}>
                <div class="wiz-task-mark">2</div>
                <div class="wiz-task-body">
                  <div class="wiz-task-title">
                    <strong>Not using a Chromium browser?</strong>
                  </div>
                  <p>
                    You can load the extension from disk instead. Staging copies the files somewhere permanent,
                    since a browser disables an unpacked extension if its folder moves.
                  </p>
                  ...
                  {/if}
                  <div class="wiz-actions">
                    <button class="ghost" disabled={setupBusy === "stage"} on:click={stageExtension}>
                      {setupBusy === "stage" ? "Staging..." : setupStatus?.extension_staged ? "Re-stage files" : "Stage files"}
                    </button>
                  ...
                  {#if setupStatus?.extension_staged}
                    <p>
                      Go to <code>{extensionsUrl}</code>, turn on <em>Developer mode</em>, choose
                      <em>Load unpacked</em>, and pick the folder above.
                    </p>
                  {/if}
                </div>
              </div>
  ...
  {#if onboardingOpen}
    <div class="modal-backdrop onboarding-backdrop" role="presentation">
      <div class="onboarding-modal" role="dialog" aria-modal="true" aria-label="KeyRail setup" tabindex="-1" on:keydown={(event) => {
        ...
        {#if onboardingCurrent === "welcome"}
          <section class="onboarding-step">
            ...
            <p>KeyRail listens for your shortcuts and runs the action you choose. This takes about a minute, and you can skip anytime and do it later in Settings.</p>
          </section>
        {:else if onboardingCurrent === "startup"}
          <section class="onboarding-step">
            <h2>Start with Windows</h2>
            <p>Start KeyRail automatically when you log in, so your hotkeys always work.</p>
        ...
        {:else}
          <section class="onboarding-step">
            ...
            <p class="onboarding-note">You can change startup later in Settings. Your config lives in %APPDATA%\KeyRail for backup or multi-PC setup.</p>
          </section>
        {/if}
        </div>
        {/key}
