...
// thread as hotkeys.
//
// Two detection backends run together. RegisterHotKey is primary: it claims the
// combo so the focused app never sees it. Raw Input (WM_INPUT with
// RIDEV_INPUTSINK) is the fallback that keeps firing when a foreground app has
// already taken the key. The registry debounces per combo so a press seen by
// both backends runs the action once.
...
static constexpr UINT WM_KEYRAIL_SUSPEND = WM_APP + 3;
static constexpr UINT WM_KEYRAIL_RESUME = WM_APP + 4;
static constexpr wchar_t kSingleInstanceMutex[] = L"Local\\KeyRailDaemon";
static constexpr wchar_t kRawInputWindowClass[] = L"KeyRailRawInputSink";
...
int main() {
    ...
    wprintf(L"control pipe: \\\\.\\pipe\\keyrail-control\n");
    ...
    while ((r = GetMessageW(&msg, nullptr, 0, 0)) != 0) {
        ...
        if (msg.message == WM_KEYRAIL_RELOAD) {
            wprintf(L"\nreload requested\n");
            loadAndApply(state, true);
            continue;
        }

        if (msg.message == WM_KEYRAIL_QUIT) {
            break;
        }

        if (msg.message == WM_KEYRAIL_SUSPEND) {
