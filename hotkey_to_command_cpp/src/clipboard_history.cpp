...
// The app that currently has focus — i.e. the source of a just-copied item.
...
std::wstring itemSubtitle(const ClipItem& item) {
    ...
    if (item.kind == ClipKind::Image) {
        if (item.width > 0 && item.height > 0) {
            if (hasApp) out << L" · ";
            out << item.width << L" × " << item.height;
        }
    } else if (isLinkText(item.text)) {
        if (hasApp) out << L" · link";
    } else {
        if (hasApp) out << L" · ";
...
void paintFooter(HDC dc, int width, int height, bool mouseMode) {
    ...
    if (mouseMode) {
        ...
        label(L"·  Scroll for more   ", textWidth(dc, L"·  Scroll for more   ", 12, FW_NORMAL) + 6);
    } else {
        x = drawKeycap(dc, x, y, L"↑") + 3;
        x = drawKeycap(dc, x, y, L"↓") + 8;
...
void paintCard(HDC dc, const RECT& card, const ClipItem& item, float hover) {
    ...
    if (item.kind == ClipKind::Image) {
        ...
        if (item.width > 0 && item.height > 0) {
            std::wstring dims = std::to_wstring(item.width) + L" × " + std::to_wstring(item.height);
